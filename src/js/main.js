...
class Quiz {
    ...
    renderResult() {
        this.divQuestion.textContent = `Квиз пройден. Правильных ответов ${this.score} из ${this.questions.length}`;
        this.divAnwers.innerHTML = "";
...
const quizStart = new Quiz('.wrapper',{
    questions: [
        ...
        {question: 'Когда празднуется главный государственный праздник — День Канады?',
        answers: [['1 июля', 1], ['4 июля', 0], ['1 января', 0] , ['13 декабря', 0]]
        }
    ],
    results: [['Стоит получше разобраться в теме', 3], ['Неплохо, но есть что подучить', 4], ['Хорошо, многое знаете', 5], ['Отличный результат', 6]]
});
